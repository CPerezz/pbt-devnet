...
// fork time T, and nothing observed at runtime changes them. The one
// dynamic rule is refusal: an op that would still be open after T-300s
...
type op struct {
	...
	victims []int
	deep    bool // aimed at depth >= 10 (heavy-victim window)
	refused string
}
...
// profileWindows are genesis-relative offsets in seconds, sized by live
// devnet evidence. A partition that diverges across a finalized
...
// clear remote bans - measured live), and the at-genesis devnet's
// `make repeer` note documents the same pathology. So no window may let
// the majority finalize while the victim diverges. The answer is stake,
// not duration: the deep victim (eligible[0], the participant holding
// 256 of 640 validators = 40%) leaves the connected majority at 60% -
...
// across three tries) - the 3-minute scale measured to heal cleanly.
// Shorts rotate over the light nodes; their 80% connected majority keeps
// finality flowing and their heal reorgs stay shallow and quick.
...
// the boundary must be crossed by a connected network .
...
func resolve(profile string, genesis, forkTime time.Time, eligible []int) (schedule, error) {
	...
	for i, w := range windows {
		...
		// outright. Compressing it instead would trade the profile's
