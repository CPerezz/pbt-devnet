...
verify-migration: bin ## verify a migration run (required: LOGS_DIR=dir BINARY_TRIE_TIME=unix; reads --el/--pins from ENCLAVE=$(ENCLAVE)/PINS=$(PINS))
	...
	@mkdir -p $(LOGS_DIR)
	kurtosis service logs $(ENCLAVE) migration-monitor > $(LOGS_DIR)/migration-monitor.jsonl 2>/dev/null || true
	kurtosis service logs $(ENCLAVE) migration-chaos > $(LOGS_DIR)/migration-chaos.jsonl 2>/dev/null || true
